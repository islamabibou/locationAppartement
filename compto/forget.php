...
if(!empty($_POST) && !empty($_POST['mail'])){
  ...
  $req = $pdo-> prepare('SELECT * FROM compte WHERE email = ? AND confirmed_at IS NOT NULL');
  $req-> execute(array($_POST['mail']));
  $user = $req-> fetch();
  if($user){
    session_start();
    $reset_token = str_random(60);
    $pdo->prepare('UPDATE compte SET reset_token = ?, reset_at = NOW() WHERE id = ?')->execute(array($reset_token, $user->id));
    ...
    mail($_POST['mail'], 'Réinitialisation de votre mot de passe', "Merci de bien vouloir cliquer sur le lien ci-après pour la réinitialisation de votre mot de passe: https://www.cmachanx.com/bureau/compte/reset.php?id={$user->id}&token=$reset_token\n\n
