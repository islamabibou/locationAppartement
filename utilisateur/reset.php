...
if(isset($_GET['id_user']) && isset($_GET['token'])){
  ...
  $req = $pdo->prepare('SELECT * FROM utilisateur WHERE id_user = ? AND reset_token IS NOT NULL AND reset_token = ? AND reset_at > DATE_SUB(NOW(), INTERVAL 30 MINUTE)');
  ...
  if($user){
    ...
    if(!empty($_POST['password']) && $_POST['password'] == $_POST['password_confirm']){
      ...
      $pdo->prepare('UPDATE utilisateur SET passe = ?, reset_at = NULL, reset_token = NULL WHERE id_user = ?')->execute(array($password, $user->id_user));
