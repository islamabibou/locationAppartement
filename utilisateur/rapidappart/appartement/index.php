    <?php
      ...
      $listeCount = "SELECT COUNT(id) as count FROM appartement";
      $params = [];
    
      //Oganisation aussi
      $sortable = ["id", "nom", "prix", "adresse", "causion", "lien"];
      ...
      $emploi = $pdo->prepare("SELECT * FROM appartement");
      $emploi->execute();
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Il y a <?= $emplois; ?> appartement<?= ($emplois > 1)? 's' : '' ?> mis sur RapidAppart</h1>
        ...
        <table class="table table-striped projects">
          <thead>
            <tr>
              ...
              <th><?= TableHelper::sort('caution', 'caution', $_GET) ?></th>
              <th>Modification</th>
              <th>Suppression</th>
            </tr>
          </thead>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><?= $liste['caution']; ?></td>
            <td>
            <a class="btn btn-outline-success" href="EditAppartement.php?id=<?= $liste['id']; ?>">Editer</a>
            </td>
            <td>
              <form action="code.php" method="post">
                <input type="hidden" name="delete_id" value="<?= $liste['id']; ?>">
                <button type="submit" class="btn btn-outline-danger" onclick = "return confirm ('Vous êtes sûr le point de supprimer l\'appartement n° <?= $liste['id']; ?>. Êtes-vous sûr de cette action?')">Supprimer</button>
              </form>
            </td>
            </tr>
              <?php endforeach ?>
